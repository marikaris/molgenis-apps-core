<template>
  <th>
    {{ label }}
    <!-- FIXME replace with fontawesome -->
    <span v-if="sortable"
...
<script>
  export default {
    ...
    methods: {
      handleSort: function () {
        const sortDirection = this.sortDirection ? (this.sortDirection === 'ascending' ? 'descending' : 'ascending') : 'ascending'
        this.$emit('sort', this.id, sortDirection)
      }
    }
  }
</script>
